...
| 文件 | 说明 |
|------|------|
| `dist/PasteNest.app` | 应用程序 |
| `dist/PasteNest-1.0.0.dmg` | 拖拽安装镜像（推荐日常分发） |
| `dist/PasteNest-1.0.0.pkg` | 可选，标准安装器（`--pkg`） |
| `dist/SHA256.txt` | 校验和 |
...
#    xcrun notarytool store-credentials PasteNestNotary \
...
1. 双击打开 `PasteNest-x.y.z.dmg`
2. 将 **PasteNest** 拖到 **Applications**
...
3. 在 Artifacts 中下载 `PasteNest-*-macos.zip`（内含 `.dmg`）
