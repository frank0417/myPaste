...
| 文件 | 说明 |
|------|------|
| `dist/ClipStack.app` | 应用程序 |
| `dist/ClipStack-1.0.0.dmg` | 拖拽安装镜像（推荐日常分发） |
| `dist/ClipStack-1.0.0.pkg` | 可选，标准安装器（`--pkg`） |
| `dist/SHA256.txt` | 校验和 |
...
1. 双击打开 `ClipStack-x.y.z.dmg`
2. 将 **ClipStack** 拖到 **Applications**
...
3. 在 Artifacts 中下载 `ClipStack-*-macos.zip`（内含 `.dmg`）
...
| `NOTARY_PROFILE` | notarytool 钥匙串配置名，默认 `ClipStackNotary` |

## 注意

- 当前 Linux / Cloud Agent 环境 **无法** 真正编译 `.app`，脚本会明确提示去 Mac 或 Actions 执行。
- 上架 Mac App Store 时请使用可区分的应用显示名（当前为 ClipStack），并配置真实 Team / iCloud 容器。
