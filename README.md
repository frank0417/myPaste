...
- **截图入库**：区域 / 窗口 / 整屏三种截图，冻结当前屏幕后拖出选区（Esc 取消），可用矩形、箭头、画笔、文字、马赛克等标注，确认后同时进入剪贴板与历史，可搜「截图」找回
- **识别文字**：截完图后在卡片上右键「识别文字」（详情页、主窗口预览也有按钮），用 Vision 在本机识别画面文字（中英文），文字附在图片上、可搜索可复制；菜单里的「截取区域并识字」则**只保留文字、不保存图片**，识别出链接会自动变成链接条目
- **全局快捷键**：底部面板、主窗口、区域截图各有一个可自定义快捷键（默认 `⇧⌘V` / `⌥⌘V` / `⇧⌘D`），在「设置 → 快捷键」录制（面板菜单与菜单栏右键菜单都有「快捷键设置…」直达入口），录下即刻生效，可一键恢复默认；组合被占用时保留原快捷键并提示。识字不占用快捷键
...
```bash
./scripts/package.sh              # 生成 dist/PasteNest.app + dist/PasteNest-1.5.7.dmg
