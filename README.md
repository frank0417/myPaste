...
- **截图文字识别**：截完立刻用 Vision 在本机识别画面文字（中英文），图片与文字一起写进剪贴板 —— 粘到输入框得到文字，粘到图片位置得到图片；识别结果存入历史，可直接搜截图里的字
- **全局快捷键**：底部面板、主窗口、区域截图各有一个可自定义快捷键（默认 `⇧⌘V` / `⌥⌘V` / `⌃⇧⌘4`），在「设置 → 通用 → 快捷键」录制；组合被占用时保留原快捷键并提示
...
6. 首次截图时，在「系统设置 → 隐私与安全性 → 屏幕录制」中允许 Paste（授权后需重启 App）
