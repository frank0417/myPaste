...
- **全局快捷键**：底部面板、主窗口、区域截图各有一个可自定义快捷键（默认 `⇧⌘V` / `⌥⌘V` / `⇧⌘X`），在「设置 → 快捷键」录制（通用页点「修改…」也会打开该页），录下即刻生效，可一键恢复默认；组合被占用时保留原快捷键并提示。识字不占用快捷键
...
```bash
./scripts/package.sh              # 生成 dist/PasteNest.app + dist/PasteNest-1.5.12.dmg
